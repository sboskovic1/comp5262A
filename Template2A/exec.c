...
void FUs(){
  ...
  while(1){
    SemaphoreWait(sem_myFU[fu]); // Wait for work
    isFree[fu] = FALSE;
    ...
    case LOAD:    
      ProcessDelay((double) LOAD_CYCLES - epsilon);   
    ...
    case STORE: 
      ProcessDelay((double) STORE_CYCLES - epsilon);
