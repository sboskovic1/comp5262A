...
void do_write() {
  ...
  for (fu=1; fu < NUM_FU * NUM_COPIES; fu++) {
    ...
    if (TRACE)
      printf("\tCompleted Instruction: %s. Result: %d Dest Reg: %d Time: %2.0f  Num Instructions Completed: %d\n", map(fu), result, destReg, GetSimTime(), numInstrComplete);  
    break;
  }
}
